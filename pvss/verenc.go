...
// does hash really need to be into [0, 2^l]?
...
func (csp *CSPaillierEC) Encrypt(m, label *big.Int) (*big.Int, *big.Int, *big.Int, error) {
	...
	hashNum := xlabcommon.Hash(u, e, label)
...
func (csp *CSPaillierEC) generateKey() {
	p1 := xlabcommon.GetGermainPrime(csp.SecParams.L)
	q1 := xlabcommon.GetGermainPrime(csp.SecParams.L)
	...
	//csp.lambda = xlabcommon.LCM(p_min, q_min)
	...
	secretKey.X1 = xlabcommon.GetRandomInt(b)
	secretKey.X2 = xlabcommon.GetRandomInt(b)
	secretKey.X3 = xlabcommon.GetRandomInt(b)

	for { // choose g1 from Z_n^2*
		g1 := xlabcommon.GetRandomInt(n2)
...
func (csp *CSPaillierEC) GetProofRandomData(u, e, label *big.Int) (*big.Int, *big.Int,
	*big.Int, *Point, *big.Int, error) {
	...
	r1, err := xlabcommon.GetRandomIntFromRange(new(big.Int).Neg(b1), b1)
	if err != nil {
		return nil, nil, nil, nil, nil, err
	}

	b2 := new(big.Int).Mul(csp.PubKey.VerifiableEncGroupN, t1)
	s1, err := xlabcommon.GetRandomIntFromRange(new(big.Int).Neg(b2), b2)
	if err != nil {
		return nil, nil, nil, nil, nil, err
	}
	...
	m1, err := xlabcommon.GetRandomIntFromRange(new(big.Int).Neg(b3), b3)
	if err != nil {
		return nil, nil, nil, nil, nil, err
	}
	...
	// u1 = g^(2*r1)
	u1 := xlabcommon.Exponentiate(csp.PubKey.G, new(big.Int).Mul(big.NewInt(2), r1), n2)

	// e1 = y1^(2*r1) * h^(2*m1)
	h := new(big.Int).Add(csp.PubKey.N, big.NewInt(1)) // 1 + n
	e1Part1 := xlabcommon.Exponentiate(csp.PubKey.Y1, new(big.Int).Mul(big.NewInt(2), r1), n2)
	e1Part2 := xlabcommon.Exponentiate(h, new(big.Int).Mul(big.NewInt(2), m1), n2)
	...
	v11.Mod(v11, n2)
	v1 := xlabcommon.Exponentiate(v11, new(big.Int).Mul(big.NewInt(2), r1), n2)

	// delta1 = gamma^m1
	// delta1 := xlabcommon.Exponentiate(csp.PubKey.Gamma.G, m1,
	// 	csp.PubKey.Gamma.P)
	delta1 := pt(csp.PubKey.Gamma.ScalarBaseMult(m1.Bytes()))

	// l1 = g1^m1 * h1^s1
	l11 := xlabcommon.Exponentiate(csp.PubKey.VerifiableEncGroupG1, m1,
		csp.PubKey.VerifiableEncGroupN)
	l12 := xlabcommon.Exponentiate(csp.PubKey.VerifiableEncGroupH1, s1,
		csp.PubKey.VerifiableEncGroupN)
...
func (csp *CSPaillierEC) Verify(rTilde, sTilde, mTilde *big.Int) bool {
	...
	t1 := xlabcommon.Exponentiate(csp.verifierEncData.U, twoC, n2)
	t2 := xlabcommon.Exponentiate(csp.PubKey.G, twoRTilde, n2)
	...
	t1 = xlabcommon.Exponentiate(csp.verifierEncData.E, twoC, n2)
	y1 := new(big.Int).Mod(csp.PubKey.Y1, n2)
	t2 = xlabcommon.Exponentiate(y1, twoRTilde, n2)
	h := new(big.Int).Add(csp.PubKey.N, big.NewInt(1)) // 1 + n
	t3 := xlabcommon.Exponentiate(h, new(big.Int).Mul(big.NewInt(2), mTilde), n2)
	t.Mul(t1, t2)
	t.Mul(t, t3)
	t.Mod(t, n2)
	...
	t1 = xlabcommon.Exponentiate(csp.verifierEncData.V, twoC, n2)
	hashNum := xlabcommon.Hash(csp.verifierEncData.U, csp.verifierEncData.E,
	...
	t21.Mul(y2, t21)
	t2 = xlabcommon.Exponentiate(t21, twoRTilde, n2)
	t.Mul(t1, t2)
	t.Mod(t, n2)
	...
	// t2 = xlabcommon.Exponentiate(csp.PubKey.Gamma.G, mTilde, csp.PubKey.Gamma.P)
	...
	t2 = xlabcommon.Exponentiate(csp.PubKey.VerifiableEncGroupG1,
		mTilde, csp.PubKey.VerifiableEncGroupN)
	t3 = xlabcommon.Exponentiate(csp.PubKey.VerifiableEncGroupH1,
